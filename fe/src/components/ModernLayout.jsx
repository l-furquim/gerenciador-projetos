...
import { BarChart3, Users, Clock, ChevronDown, Calendar, X, Edit, Trash2, FolderOpen } from 'lucide-react'
...
export default function ModernLayout({ 
  ...
  const handleSaveEdit = () => {
    ...
    setDevelopers(prev => prev.map(dev => 
      dev.id === editDeveloper?.id 
        ? { ...dev, ...editForm, hourlyRate: parseFloat(editForm.hourlyRate) }
        : dev
    ))
    
    // Atualizar o desenvolvedor selecionado se for o mesmo que está sendo editado
    if (selectedDeveloper?.id === editDeveloper?.id) {
      setSelectedDeveloper({ ...editDeveloper, ...editForm, hourlyRate: parseFloat(editForm.hourlyRate) })
    }
